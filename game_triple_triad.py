    ...
    def maybe_rules_multiple_takes(self, neighbors, card):
        if cell_top := neighbors.get('top'):
            if cell_left := neighbors.get('left'):
                if self.compare_rule(card.top, cell_top['card'].bottom, card.left, cell_left['card'].right):
            ...
            if cell_right := neighbors.get('right'):
                if self.compare_rule(card.top, cell_top['card'].bottom, card.right, cell_right['card'].left):
                    self.maybe_update_cell_take_over_rule(cell_top)
                    self.maybe_update_cell_take_over_rule(cell_right)

            if cell_bottom := neighbors.get('bottom'):
                if self.compare_rule(card.top, cell_top['card'].bottom, card.bottom, cell_bottom['card'].top):
                    self.maybe_update_cell_take_over_rule(cell_top)
                    self.maybe_update_cell_take_over_rule(cell_bottom)

        if cell_left := neighbors.get('left'):
            if cell_right := neighbors.get('right'):
                if self.compare_rule(card.left, cell_left['card'].right, card.right, cell_right['card'].left):
                    self.maybe_update_cell_take_over_rule(cell_left)
                    self.maybe_update_cell_take_over_rule(cell_right)

            if cell_bottom := neighbors.get('bottom'):
                if self.compare_rule(card.left, cell_left['card'].right, card.bottom, cell_bottom['card'].top):
                    self.maybe_update_cell_take_over_rule(cell_left)
                    self.maybe_update_cell_take_over_rule(cell_bottom)

        if cell_right := neighbors.get('right'):
            if cell_bottom := neighbors.get('bottom'):
                if self.compare_rule(card.right, cell_right['card'].left, card.bottom, cell_bottom['card'].top):
                    self.maybe_update_cell_take_over_rule(cell_right)
                    self.maybe_update_cell_take_over_rule(cell_bottom)
